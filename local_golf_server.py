...
def login_page():
    if current_user():
        return redirect('/')
    gui_path = os.path.join(BASE_DIR, 'local_golf_gui.html')
...
def get_events():
    try:
        ...
        script_path = os.path.join(BASE_DIR, 'local_golf_fetch_events.py')
...
def get_bookings():
    try:
        import json as _json
        script_path = os.path.join(BASE_DIR, 'local_golf_fetch_bookings.py')
        result = subprocess.run([sys.executable, '-u', script_path],
                                capture_output=True, text=True, timeout=60,
                                env=creds_env())
        for line in result.stdout.splitlines():
...
def run_script():
    ...
    def run():
        script_path = os.path.join(BASE_DIR, 'local_roseville_golf_booking.py')
...
def get_members():
    try:
        script_path = os.path.join(BASE_DIR, 'local_golf_fetch_members.py')
